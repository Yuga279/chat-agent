...
/**
 * Phase-1 smoke test: proves the AG-UI <-> LangGraph wiring works end-to-end before any
 * CopilotKit/React involvement (Phase 2+). Requires two things running first:
 *
 *   1. `npm run graph:dev` (in this directory) - starts the LangGraph.js dev server on
 *      port 2024, serving the "research" graph from langgraph.json.
 *   2. MongoDB and the System1.MCP server up, same as for `npm start` (the graph's tool
 *      list comes from the same buildTools()/memory-tools path as the existing agents).
 *
...
async function main() {
  ...
  console.log(`Running research graph for externalUserId=${externalUserId!}...\n`);
